...
def lambda_handler(event, context):
    ...
    Creates VPC with public and private subnets (with ONE NAT Gateway) spread across all Availability Zones.
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"CIDR of VPC {vpc_cidr} it's too small to be divided in two. Error: {e}")
        ...
        if public_subnet_prefix > 28:
             raise ValueError(f"Many AZs ({az_count}) to divide the block {public_cidr_block}. The subnets would be smaller than /28.")

        public_subnets_iter = public_cidr_block.subnets(new_prefix=public_subnet_prefix)
        ...
        for i, az_name in enumerate(availability_zones):
            ...
            add_tags(pub_subnet_id, f"{PROJECT_TAG}-{job_id}-Public-Subnet-{i+1}-{az_name}")
            
            ec2_client.associate_route_table(RouteTableId=public_rt_id, SubnetId=pub_subnet_id)
            ec2_client.modify_subnet_attribute(SubnetId=pub_subnet_id, MapPublicIpOnLaunch={'Value': True})
            public_subnet_ids.append(pub_subnet_id)
            logger.info(f"Public Subnet {pub_subnet_id} ({public_subnet_cidr}) on AZ {az_name} created.")
            ...
            add_tags(priv_subnet_id, f"{PROJECT_TAG}-{job_id}-Private-Subnet-{i+1}-{az_name}")
            private_subnet_ids.append(priv_subnet_id)
            logger.info(f"Private Subnet {priv_subnet_id} ({private_subnet_cidr}) on AZ {az_name} created.")

        # --- 8. Create the SINGLE NAT Gateway ---
        if not first_public_subnet_id:
            raise ValueError("No public subnet was created to host the NAT Gateway.")
        ...
        logger.info(f"SINGLE NAT Gateway {nat_gw_id} creating on subnet {first_public_subnet_id}...")
        ...
        logger.info(f"All {len(private_subnet_ids)} private subnets association on private RT {private_rt_id}.")
        
        # --- 10. Wait for NAT Gateway and Add Single Private Route ---
        try:
            logger.info(f"Waiting NAT Gateway {nat_gw_id} to become 'available'...")
            ...
            raise Exception(f"NAT Gateway {nat_gw_id} failed to become 'available'.")
...
def update_status(job_id, status, results=None, error_message=None):
    ...
    try:
        ...
        logger.error(f"It was not possible to update the status in DynamoDB for the job {job_id}: {e}")